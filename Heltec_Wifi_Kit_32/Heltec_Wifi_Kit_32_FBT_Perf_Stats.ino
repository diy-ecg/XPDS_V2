...
void loop() {
  ...
  if (currentMillis - lastBatteryCheck >= batteryCheckInterval) {
    ...
    // Check Batteries
    int batValue = analogRead(ADC_BATTERY_PIN);
    int vgValue  = analogRead(ADC_U2_PIN);
    ...
    while (line != nullptr) {
      if (strstr(line, "IDLE0")) {
        sscanf(line, "IDLE0 %*u %f", &idle0);
      } else if (strstr(line, "IDLE1")) {
        sscanf(line, "IDLE1 %*u %f", &idle1);
      }
      line = strtok(nullptr, "\n");
    }
    char coreString[16];
    if (idle0 >= 0 && idle1 >= 0) {
      ...
      snprintf(coreString, sizeof(coreString),"C0:%.1f%%|C1:%.1f%%\n", core0Usage, core1Usage);
    } 
    else {
      snprintf(coreString, sizeof(coreString),"No IDLE-Task");
      core0Usage = core1Usage = -1;
    }
    // Check Heap Memory
    uint32_t freeHeap = ESP.getFreeHeap();
    char memStr[32];
    snprintf(memStr, sizeof(memStr), "Heap: %lu", freeHeap);
    // Update LCD Display
    u8g2.clearBuffer();
    u8g2.drawStr(0, 10, "WiFi connected");
    u8g2.drawStr(0, 20, "ADS1115 connected");
